...
('KH01', N'Phạm Văn Bình', '079123456789', N'Việt Nam', '0912345678'),
('KH02', N'Lê Thị Cẩm', '079987654321', N'Việt Nam', '0987654321');

INSERT INTO PhieuDatPhong(SoPhieuDat, MaKhach, MaNVLeTan, NgayLap, NgayNhan, NgayTraDuKien, TienCoc, KenhDat, TrangThai, NgayNhanThucTe) VALUES
('DP001', 'KH01', 'NV01', GETDATE(), CAST(GETDATE() AS date), DATEADD(day,2,CAST(GETDATE() AS date)), 200000, N'Trực tiếp', N'Đang ở', GETDATE()),
('DP002', 'KH02', 'NV01', GETDATE(), CAST(GETDATE() AS date), DATEADD(day,1,CAST(GETDATE() AS date)), 300000, N'Website', N'Đang ở', GETDATE());

INSERT INTO ChiTietDatPhong(SoPhieuDat, SoPhong, SoNguoi) VALUES
('DP001', 'A101', 2),
('DP002', 'B201', 3);

-- Cập nhật trạng thái phòng cho khớp
UPDATE Phong SET TrangThai = N'Đang ở' WHERE SoPhong IN ('A101','B201');
GO
SELECT * FROM ChiTietDatPhong WHERE SoPhieuDat = 'DP001';