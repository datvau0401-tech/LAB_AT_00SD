...
namespace QuanLyKhachSan.Services
{
    public class DatPhongService
    {
        ...
        {
            return Db.Query(
                "SELECT c.*, p.SoNguoiToiDa, p.DonGiaNgay " +
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    foreach (var x in ds)
                    {
                        ...
                        {
                            return KetQuaXuLy.Fail(
                                "Không tìm thấy phòng " + x.SoPhong
                            );
                        }

                        if (x.SoNguoi <= 0 ||
                            x.SoNguoi > Convert.ToInt32(o))
                        {
                            return KetQuaXuLy.Fail(
                                "Số người của phòng " +
                                x.SoPhong +
                    ...
                    var h = new SqlCommand(
                        ...
                        "(SoPhieuDat, MaKhach, MaNVLeTan, NgayLap, " +
                        "NgayNhan, NgayTraDuKien, TienCoc, KenhDat, TrangThai) " +
                        "VALUES(@s, @k, @nv, @lap, @nhan, @tra, " +
                        "@c, @kenh, N'Đã đặt')",
                        cn,
                        tx
                    );
            ...
            string qt)
        {
            ...
            try
            {
                ...
                Db.Execute(
                    ...
                    "(SoPhieuDat, SoPhong, HoTen, SoCMND, QuocTich) " +
                    "VALUES(@s, @p, @t, @c, @q)",
                    new SqlParameter("@s", so),
                    new SqlParameter("@p", phong),
                    new SqlParameter("@t", ten),
