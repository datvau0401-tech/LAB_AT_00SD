using QuanLyKhachSan.Services.Data;
using System;
...
namespace QuanLyKhachSan.Services
{
    public class TraPhongService
    {
        ...
        {
            return Db.Query(
                "SELECT d.SoPhieuDat, k.HoTen, " +
                "d.NgayNhanThucTe, d.NgayTraDuKien " +
        ...
        public KetQuaXuLy LapPhieuDenBu(
            string soDB,
            string soDat,
            string phong,
            DateTime ngay,
            ...
            List<DenBuItem> ds)
        {
            if (string.IsNullOrWhiteSpace(soDB) ||
                string.IsNullOrWhiteSpace(soDat) ||
                ...
                ds.Count == 0)
            {
                return KetQuaXuLy.Fail(
                    "Phiếu đền bù chưa đủ thông tin."
                );
            }
            ...
            int soNgay)
        {
            ...
            {
                return KetQuaXuLy.Fail(
                    "Thông tin hóa đơn chưa hợp lệ."
                );
            }

            try
            {
                decimal phong = Convert.ToDecimal(
                    Db.Scalar(
                ...
                Db.Execute(
                    "INSERT INTO HoaDon " +
                    "(SoHoaDon, SoPhieuDat, NgayLap, MaNV, " +
                    "SoNgayTinhTien, TienPhong, TienDichVu, TrangThai) " +
                    "VALUES(@h, @s, @n, @nv, @ng, @p, @d, " +
                    "N'Chưa thanh toán')",
                ...
                return KetQuaXuLy.Ok(
                    "Đã lập hóa đơn tiền phòng và dịch vụ. " +
                    "Số ngày tính tiền là dữ liệu nhân viên xác nhận " +
                    "vì đề gốc không quy định cách làm tròn ngày lưu trú."
                );
            }
            catch (Exception ex)
            {
            ...
            decimal tien)
        {
            if (string.IsNullOrWhiteSpace(maTT) ||
                string.IsNullOrWhiteSpace(soHD) ||
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        return KetQuaXuLy.Fail(
                            "Số tiền thanh toán vượt số còn phải trả."
                        );
                    }

                    var i = new SqlCommand(
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    var u2 = new SqlCommand(
                        ...
                        "WHERE SoPhong IN " +
                        "(SELECT SoPhong FROM ChiTietDatPhong " +
                        "WHERE SoPhieuDat = @s)",
                        cn,
                        tx
                    );

                    u2.Parameters.AddWithValue(
                        "@s",
                        soDat
                    );
